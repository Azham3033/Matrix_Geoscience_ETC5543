...
ui <- fluidPage(
  ...
  tags$head(
    ...
    tags$style(HTML("
      /* Dynamic vars used: WMS height + table scroller height */
      :root { --mapH1: 360px; --tableH: 420px; } /* defaults before JS runs */
      ...
      /* Fit-to-screen: WMS dynamic; table scroller dynamic; static plot fixed via plotOutput() */
    ...
    # -------- JS: allocate WMS + table heights; static plot is fixed height --------
    tags$script(HTML("
      (function() {
        function fitHeights() {
          ...
          var buffer   = 40;   // breathing room
          var STATIC_H = 520;  // must match plotOutput('map_static', height = 520)

          // Remaining vertical space for WMS + table
          var avail = Math.max(300, vh - b - c - buffer - STATIC_H);

          // Allocate ~55% of 'avail' to the WMS; rest to table
    ...
    # Reset → full reload
    tags$script(HTML("
      Shiny.addCustomMessageHandler('reload_page', function(x) { location.reload(); });
    "))
  ),
  ...
  fluidRow(
    ...
    column(
      ...
      leafletOutput("map_wms", height = 100),  # CSS overrides via --mapH1
      br(), h5("Static zoomed parcel"),
      plotOutput("map_static", height = 520)   # FIXED height; not resized by JS/CSS
    )
  )
)
  ...
  output$tbl <- renderDT(server = FALSE, {
    ...
    datatable(
      ...
      options = list(
        ...
        scrollX = TRUE,
        scroller  = TRUE,
        scrollY   = 1,               # enable Scroller; real height via CSS var
