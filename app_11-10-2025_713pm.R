...
ui <- fluidPage(
  ...
  tags$head(
    ...
    tags$style(HTML("
      ...
      /* Full-bleed top banner: spans to the screen edges */
      .app-banner-strip {
        width: 100vw;
      ...
      .bar-2 {
        ...
        flex-wrap: wrap;
        gap: 10px;
        justify-content: flex-end;   /* right-align group */
        align-items: end;
        margin-top: 8px;
      }
      ...
      .bar-1 input, .bar-1 .selectize-input,
      .bar-2 input, .bar-2 .selectize-input { min-height: 32px; }

      #basemap { min-width: 260px; }
      #addr    { min-width: 260px; }
      #limit   { width: 120px; }

      /* Prevent shrinking on Row 2; consistent buttons */
      .bar-2 > * { flex: 0 0 auto; }
      .bar-2 .btn { min-width: 88px; padding: 6px 14px; }

      /* Lime-green Reset button */
      ...
      /* Help icon “?” next to Basemap */
      .help-icon {
        display: inline-block; margin-left: 6px;
        width: 18px; height: 18px; line-height: 18px; text-align: center;
        border-radius: 50%; background: #e9ecef; color: #333; font-weight: 700;
        cursor: pointer; font-size: 12px;
      }

      /* Red star (hover tooltip on Page 1, decorative on Page 2 Email) */
      .req-star { color: #d93025; margin-left: 6px; cursor: pointer; font-weight: 700; }
      label[for='reg_email'] .req-star { cursor: default; } /* non-clicky look on Page 2 */
      ...
      .spinner-ring {
        ...
        border: 6px solid #999; border-top-color: transparent;
    ...
    # -------- JS: dynamic heights & tooltip init --------
    tags$script(HTML("
      (function() {
        function fitHeights() {
        ...
        // Bootstrap tooltip init (BS3/BS5 attributes)
        function initTips(){
        ...
        $(document).on('shiny:value', initTips);
        $(document).on('shiny:inputchanged', initTips);
      })();
    ")),
  ...
  tabsetPanel(
    ...
    tabPanel(
      ...
      div(
        ...
        # ROW 1: target/preference filters
        div(
          class = "bar-1",
          uiOutput("cropcat_ui"),
          uiOutput("crop_ui"),
        ...
        # ROW 2: right-aligned, left→right = Basemap | Run | Reset
        div(
          class = "bar-2",
          selectInput(
            "basemap",
            tagList(
              ...
              tags$span(
                ...
                title = paste(
                  "CartoDB Positron: light, street-focused cartography.",
                  "Topographic (LIST): theLIST’s official topographic base.",
                  "Orthophoto: high-resolution aerial/satellite imagery.",
                  sep = " "
                )
              )
            ),
    ...
    tabPanel(
      ...
      fluidRow(
        column(
          ...
          textInput(
            "reg_email",
            tagList("Email address", tags$span("*", class = "req-star")),
          ...
          selectInput(
            "reg_class", "Class label",
            choices = c(
              "", "1.0 Well suited", "1.1 Well suited (with soil mgmt)", "2.0 Suitable",
...
server <- function(input, output, session) {
  ...
  cropcat_choices <- reactive({
    ...
    vals <- dbGetQuery(con, sprintf(
      "SELECT DISTINCT CROP_CATEGORY FROM %s ORDER BY 1",
      dbQuoteIdentifier(con, CROP_MASTER)
    ))
    c("All", vals[[1]])
  })
  ...
  class_choices <- reactive({
    ...
    vals <- dbGetQuery(con, sprintf(
      "SELECT DISTINCT CLASS_LABEL FROM %s ORDER BY 1",
      dbQuoteIdentifier(con, MAIN_TABLE)
    ))
    c("All", vals[[1]])
  })
  ...
  muni_choices <- reactive({
    req(dbExistsTable(con, MAIN_TABLE))
    vals <- dbGetQuery(con, sprintf(
      "SELECT DISTINCT MUNICIPALITY FROM %s ORDER BY 1",
      dbQuoteIdentifier(con, MAIN_TABLE)
    ))
    vals[[1]]
  })
    ...
    selectInput(
      ...
      tagList(
        ...
        tags$span(
          ...
          title = "Required if No Full Address"
        )
      ),
  ...
  build_sql <- reactive({
    ...
    if (!is.null(input$class) && input$class != "All") {
      where <- c(where, sprintf("UPPER(%s) = UPPER(?class_label)", dbQuoteIdentifier(con, "CLASS_LABEL")))
      params$class_label <- input$class
    }
    
    if (is_present(input$municipality)) {
      where <- c(where, sprintf("UPPER(%s) = UPPER(?municipality)", dbQuoteIdentifier(con, "MUNICIPALITY")))
      params$municipality <- input$municipality
    }
    
    if (is_present(input$addr)) {
      where <- c(where, sprintf("UPPER(%s) ILIKE '%%' || UPPER(?addr) || '%%'", dbQuoteIdentifier(con, "FULL_ADDRESS")))
      params$addr <- input$addr
    }
    
    where_sql   <- if (length(where)) paste("WHERE", paste(where, collapse = " AND ")) else ""
    select_cols <- paste(
      dbQuoteIdentifier(con, c("FULL_ADDRESS","MUNICIPALITY","CROP","CLASS_LABEL","PRICE","COUNT","WEBSITE")),
      collapse = ", "
    )
    
    sql <- sprintf(
      "SELECT %s FROM %s %s ORDER BY FULL_ADDRESS LIMIT %d",
  ...
  # Row click → fill address AND set CROP; also clear CLASS_LABEL
  ...
  output$map_static <- renderPlot({
    ...
    # --- Raster cutout around parcel bbox ---
    ...
    # --- Read VAT labels & attach levels (if exist) ---
    ...
    # --- Palette from your registry (labels + hex) ---
    pal     <- get_crop_palette(crop_nm)
    classes <- pal$class_label
    cols    <- pal$color_hex
    
    # Normaliser to match labels robustly
    canon2 <- function(s) {
      s <- gsub("\\s+", " ", trimws(s))
      s <- gsub("soil mgmt", "soil management", s, ignore.case = TRUE)
      tolower(s)
    }
    
    # Create label->colour LUT, then ID->colour map via VAT
    col_map <- NULL
    if (!is.null(cat_tbl)) {
      lut_lab2col <- setNames(cols, canon2(classes))           # label -> colour
      cm <- lut_lab2col[canon2(cat_tbl$label)]                 # colour per VAT label
      cm[is.na(cm)] <- "lightgray"
      names(cm) <- as.character(cat_tbl$ID)                    # index by numeric VAT Value
      col_map <- cm
    }
    
    # --- IMPORTANT: reproject parcel to raster CRS BEFORE mask ---
    parcel_r <- st_transform(parcel_sf, st_crs(r_cut))
    
    # Mask to parcel, then plot by numeric IDs
    ...
    # ---- Plot (parcel-only pixels) + legend ----
    ...
    terra::plot(
      r_fac,
      col    = cols_for_levels,     # aligned to numeric IDs in levels(r_fac)
      legend = FALSE,
  ...
  reg_crop_choices <- reactive({
    req(dbExistsTable(con, CROP_MASTER))
    vals <- dbGetQuery(con, sprintf(
      "SELECT DISTINCT CROP_NAME FROM %s ORDER BY 1",
      dbQuoteIdentifier(con, CROP_MASTER)
    ))
    c("", vals[[1]])
  })
