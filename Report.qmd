...
author:
  ...
  - name: Dr. Kane Maxwell
    degrees: Matrix Geoscience
...
format:
  report-pdf:
    ...
    title-block-author-details: true
header-includes:
  - |-
    % ---- Biblatex formatting options (DOI & URL) ----
    ...
    % ---- Disable template's auto \printbibliography; expose a controlled alias ----
    \makeatletter
    % Save original
    \let\myprintbibliographyOrig\printbibliography
    % Turn ALL later \printbibliography calls into a no-op (no xparse needed)
    \renewcommand{\printbibliography}[1][]{}
    % Provide a deliberate printer you can call once
    \newcommand{\forceprintbibliography}[1][]{\myprintbibliographyOrig[#1]}
    \makeatother
---
...
This study introduces AgriSuit, an R Shiny web dashboard that connects crop-suitability maps with real-estate listings to support quick and transparent agricultural property screening. The platform analyses official suitability data to identify the main environmental factors that limit growth and calculates the percentage of land suitable for each crop within a parcel. These results are summarised visually, helping users understand which crops are most viable and why. Through its interactive dashboard, users can explore parcel-level suitability maps, view soil and climate details, and open property listings directly to assist in due-diligence and investment decisions. In pilot use, AgriSuit delivered clear and accessible results for non-technical users while revealing current limitations, such as a fixed list of crops and simplified climate assumptions. Future development will focus on adding climate scenario options, uncertainty indicators, and water-access and economic layers to make the tool more comprehensive and scalable across regions.
...
Tasmania was chosen as the starting point for AgriSuit because it offers the right mix of economic importance, data quality, and regional focus needed to demonstrate the model’s potential, with current state accounts and recent sector scorecards indicating agriculture’s substantial contribution to the economy and continued growth momentum [@tenpercentgsp]. Agriculture plays a much larger role in Tasmania’s economy than in most Australian states, and the Agri-Food Scorecard shows the sector’s gross value rising in 2022–23, underscoring relevance for piloting decision tools that can strengthen productivity and regional planning [@howlett2025]

A key advantage of working in Tasmania is the availability of the Tasmanian Enterprise Suitability Model (TESM), which applies a rule-based most limiting factor approach and is delivered at approximately 80 m resolution suitable for farm and regional assessments. TESM delivers locally calibrated, rule-based suitability at higher resolution with explicit most-limiting-factor logic, enabling parcel-level recommendations aligned to Tasmanian conditions. TESM and related Digital Soil Assessment resources explicitly combine soil attributes with climate variables to produce categorical suitability classes, enabling transparent traceability from factor thresholds to parcel-level outcomes. Tasmania also stands out for its open access to high-quality spatial data via the LIST (Land Information System Tasmania), which provides integrated land and property information and public interfaces (e.g., LISTdata and LISTmap) used by industry and the general public [@tasmania_list_2025]. 

Altogether, positioning AgriSuit in Tasmania demonstrates the model under a data-rich, fine-resolution regime, thus makes Tasmania the most suitable region to pilot AgriSuit and demonstrate how localized, fine-resolution suitability mapping can transform land-use and crop planning decisions when connected to real-time property information.


## The Tasmanian Enterprise Suitability Map (TESM)

The [Tasmanian Enterprise Suitability Map (TESM)](https://nre.tas.gov.au/agriculture/investing-in-irrigation/enterprise-suitability-toolkit/enterprise-suitability-maps) represents a mature, data-driven framework for assessing agricultural potential across Tasmania. Originating from the [Wealth from Water](https://www.flinders.tas.gov.au/client-assets/images/Council/Downloads/Agendas/2015.02/Annex%206.%20Item%20A1%20Wealth%20from%20Water%20Brochure.pdf) program initiated in 2010 by the Tasmanian Department of Primary Industries, Parks, Water and Environment, which has since been restructured and renamed as the Department of Natural Resources and Environment Tasmania [(NRE Tas)](https://nre.tas.gov.au/) in collaboration with the Tasmanian Institute of Agriculture (TIA) and the University of Sydney, TESM sought to provide an objective, reproducible, and spatially explicit basis for agricultural development planning. The program was itself a direct response to the Innovation Strategy by @west2009, which identified irrigation expansion as a transformative opportunity for Tasmania’s regional economy. The pilot phase initially covered about 70,000 hectares in the Meander Valley and Midlands, subsequently expanding to state-wide coverage through initiatives such as [Water for Profit](https://www.utas.edu.au/__data/assets/pdf_file/0008/1092068/TIA-Water-for-Profit-Brochure-2.pdf). In this progression, the emphasis shifted from qualitative soil surveys to Digital Soil Mapping (DSM) and Digital Soil Assessment (DSA) techniques that operationalised soil and climate data into decision-support tools for land managers and investors [@kidd2020].

TESM’s analytical foundation rests upon the integration of three primary data domains, consisting of soil, climate, and topography. The domains are processed through quantitative spatial modelling. The soil component draws on more than 900 core samples describing attributes such as pH, clay content, drainage class, and stone content [@kiddwebb2015b]. [Climate inputs](https://nrmdatalibrary.nre.tas.gov.au/FactSheets/WfW/ListMapUserNotes/Inventory_DCM_Tas.pdf) originate from a network of over 270 temperature sensors distributed across representative terrain positions, enabling fine-scale modelling of variables such as frost risk, chill hours, growing-degree days, and extreme heat frequency. Topographic variables, derived from Shuttle Radar Topography Mission (SRTM) data, gamma-radiometric imagery, and surface geology, capture slope, aspect, and elevation effects. These heterogeneous datasets are standardised and co-registered, then resampled at 80 m and modelled using regression-tree and regression-kriging approaches to produce spatially continuous soil-attribute predictions with validation accuracy appropriate for regional-scale land evaluation. The result is a three-dimensional digital representation of the Tasmanian soil landscape that supports downstream suitability analyses. The extent of these variables can be viewed on @tbl-allvariables
...
In summary, deterministic, probabilistic, and hybrid frameworks each offer distinct trade-offs in interpretability, risk communication, and scalability. The underlying idea of integrating suitability evidence with property listings to support investment decisions is broadly applicable, but delivering it in practice requires custom schema mapping, legend normalisation, and jurisdiction-specific rule packs. The next section turns to GIS-enabled Spatial Decision Support Systems (SDSS), showing how interactive platforms operationalise these requirements through reproducible overlays, parcel-level summaries, and scenario/uncertainty toggles appropriate for end user workflows.

## Geographic Information Systems (GIS) and Spatial Decision Support

Geographic Information Systems (GIS) underpin the analytical foundation of this project by enabling the integration and visualization of diverse spatial data, environmental suitability layers from the Tasmanian Enterprise Suitability Model (TESM), and property attributes from real-estate listings within a unified decision environment. In the context of this study, GIS functions not as a broad mapping tool but as a decision-support mechanism, linking agronomic intelligence with market accessibility to inform sustainable agricultural investment. GIS facilitates multi-layer spatial analysis and delivers parcel-level insights critical to evidence-based land use decision making [@sharifi1994].

Spatial Decision Support Systems [(SDSS)](https://atlas.co/glossary/spatial-decision-support-systems/) extend traditional GIS capabilities by embedding rule-based and multi-criteria evaluation (MCE) models within interactive interfaces. As @carver2001 noted, the evolution from stand-alone GIS models to participatory, web-based SDSS frameworks enhanced transparency and stakeholder engagement in spatial planning. This same paradigm underpins the present project’s design, operationalising TESM data through a web-based dashboard that allows distributed users (e.g., farmers, investors, and policymakers) to explore suitability and property information simultaneously [@rinner2023].

The selection of a Shiny-based GIS interface reflects both technical and strategic considerations. Shiny provides an open-source, inter-operable, and reproducible environment within the R ecosystem, where the TESM datasets, statistical models, and spatial processing workflows already reside [@chang2023shiny]. This ensures seamless integration with R spatial packages such as sf, terra, and leaflet, supporting dynamic map rendering, spatial querying, and user-driven filtering without external proprietary dependencies. Shiny’s reactive programming structure also enables real-time interaction between crop suitability layers and property attributes, allowing users to apply filters (e.g., crop type and municipality) and instantly visualise relevant parcels and listings. Shiny’s accessibility and documentation facilitate rapid prototyping and deployment of interactive agricultural dashboards, as demonstrated by numerous applications internationally [@appsilon2025].

Open-source precedents such as [Pi-VAT](https://github.com/devalc/Pi-VAT) by @deval2022 and ExActR from @gibbons2025 demonstrate that R-based, web-enabled GIS systems can effectively translate complex environmental models into accessible decision dashboards. However, the current project extends these applications beyond environmental management to the agri-real-estate domain, embedding suitability intelligence (AgriSuit) directly within property search and investment decision workflows. By operationalising TESM data through Shiny, the system transforms GIS from a back-end analytical process into a front-end, interactive decision interface, aligning with the project’s objective of bridging agronomic insight and market opportunity within a transparent, web-accessible platform [@keenan2019].
...
DuckDB has performance speed that suitable for exploratory development and demonstrations, since it provides low-latency analytical queries (e.g., parcel-level pixel aggregations, filterable property views) and seamless input/output with CSV, enabling simple, scriptable data exchange across pipeline stages [@kohn2022]. 

Constraints of the prototype. DuckDB is not optimised for concurrent multi-user access, role-based security, or high availability. As usage scales, risks emerge around contention, access control, backup/restore guarantees, and operational observability [@mozzillo2025]. A production instance should migrate to a stand-alone database server or managed cloud service.


## Real-Estate Listing Platforms and Agricultural Land Information 

Mainstream Australian real estate platforms typically emphasize transactional details such as parcel size and tenure but seldom include verified biophysical data like soil chemistry, drainage class, frost metrics, or slope [@nationalcouncil2025]. When “suitable crops” are mentioned, these are largely anecdotal, seller-supplied claims with no reference to formal datasets or transparent suitability rules, requiring buyers to manually cross-reference external soil and climate sources [@bhaker2025]. This fragmented process systematically disadvantages non-GIS specialists who cannot easily assemble soil, climate, and topography evidence to support investment or planning decisions [@lee2023]. Studies highlight a broader lack of standardized, integrated agricultural datasets across property platforms, perpetuating risks of misinformation and reduced market transparency [@dumbrell2024]. The absence of scientifically grounded suitability layers fundamentally undermines credible decision-making in agricultural land investment.

Existing literature and planning frameworks confirm that this fragmentation is systemic. @snelder2023 emphasise that land-use suitability is not an intrinsic property of a parcel but the outcome of composite indicators like soil, terrain, climate, management, and environmental objectives, weighted according to regional priorities. Without transparency in these indicators or access to underlying data, any claim of “crop suitability” remains contestable. Similarly, governance materials such as Tasmania’s Agricultural Land Mapping Project (ALMP) and the associated zoning guidelines recognise that automated desktop models can yield anomalies and must be interpreted alongside expert review and verified datasets, including the Land Potentially Suitable for Agriculture layer and water-resource portals. These sources reinforce a shared message: sound agricultural assessment depends on integrated, multi-layer spatial evidence, not static property metadata. 

The key challenge hindering the integration of advanced land-suitability models with real estate listings lies in institutional and policy barriers that prevent spatial datasets from being shared or aligned across agencies and platforms [@ma2024]. Critical agricultural, water, and zoning information is managed separately from property marketplaces, resulting in fragmented data flows that leave buyers without access to verified, indicator-driven suitability evidence [@adams2022]. This disconnect between technical data and transactional platforms preserves a systemic gap in market transparency and inhibits evidence-based agricultural investment decisions.

AgriSuit is designed to close this gap. By directly linking TESM-derived suitability data with live property listings in an interactive interface, it connects verified spatial intelligence with market information and demonstrates how both technical and governance barriers can be overcome in practice.
...
Following the data-fetching procedures detailed in Section 6.1, the workflow transitioned into the data-modelling and integration phase, where the collected datasets were validated, cleaned, and spatially structured to form a coherent analytical foundation for the AgriSuit dashboard. This stage was essential for transforming diverse and semi-structured inputs ranging from web-scraped property listings to official cadastral parcels, into a relationally connected database capable of supporting parcel-level analysis.

### Data Cleaning and Validation

The initial step involved refining and validating the property listing and address datasets to ensure accuracy, completeness, and consistency. Raw listings retrieved from the web-scraping workflow contained missing fields, duplicated entries, and irregular formatting that could interfere with spatial joins. These records were systematically filtered, retaining only complete entries with verified location information. Textual inconsistencies such as spacing, capitalisation, and punctuation were corrected to align with Tasmanian addressing conventions. Each valid record was then geocoded to obtain latitude and longitude coordinates, ensuring positional accuracy. This process established a verified and uniform property dataset suitable for integration with official spatial layers.

### Address Standardisation

After validation, the address data were standardised to ensure structural uniformity across all datasets. Individual address components, such as street number, street name, street type, suburb, state, and postcode, were concatenated into a composite FULL_ADDRESS field. This field provided a consistent textual identifier across both the web-scraped listings and the verified address-point dataset described in Section 6.1. The purpose of this step was to harmonise address representation rather than immediately establish relational dependencies, reducing ambiguity when cross-referencing between independent data sources. The outcome was a clean, standardised address framework that ensured one-to-one correspondence between listed properties and their official spatial references.

### Parcel Preparation and Spatial Validation

Parallel to address standardisation, cadastral parcel layers were prepared and validated. Each municipal dataset contained standard GIS vector components, including geometry files (.shp), attribute tables (.dbf), coordinate definitions (.prj), and supporting index files (.shx, .sbn, .sbx). These files collectively define parcel boundaries and their associated attributes such as Parcel Identifier (PID) and Municipality_ID. To maintain uniformity, all datasets were re-projected into the common coordinate reference system EPSG:28355 (GDA94 / MGA Zone 55). Geometry checks ensured that parcel boundaries were topologically valid and spatially aligned with their corresponding address points. The PID field was reviewed to confirm uniqueness, thereby preventing duplication or missing identifiers. By the conclusion of this step, the cadastral data were spatially coherent, attribute consistent, and ready to serve as the geometric foundation for integration.

### Relational Database Design

With clean and verified datasets established, the workflow progressed to database modelling. A relational schema was designed to manage the interconnected property, address, and parcel data efficiently and to support reproducible spatial joins. The schema comprised five core tables: Municipality_Master, Address_Master, Property_Listing, Property_Master, and CROP_PROPERTY_COUNT_MASTER.
The Municipality_Master table stored parcel geometries and associated identifiers. The Address_Master table contained verified addresses linked to parcels via PID, while Property_Listing stored the cleaned Domain.com records indexed by FULL_ADDRESS. The Property_Master table functioned as the central bridge, combining PID, FULL_ADDRESS, and Municipality_ID into unified property records. The CROP_PROPERTY_COUNT_MASTER table was reserved for analytical outputs generated later during the suitability computation stage.
Each table was structured with explicit primary and foreign key constraints to ensure referential integrity. For instance, PID acted as the primary key within Municipality_Master and a foreign key within Address_Master and Property_Master. Similarly, FULL_ADDRESS served as the primary key in Property_Listing and as a foreign key reference within Property_Master. These relationships established the logical hierarchy connecting textual property identifiers to their spatial representations.

### Join Logic and Integration

Following schema design, relational joins were performed to merge the datasets into a single queryable structure. The first join linked Property_Listing and Address_Master via FULL_ADDRESS, creating verified address-to-listing correspondences. The second join connected Address_Master to Municipality_Master through the PID attribute, binding each address to its cadastral parcel boundary. The resulting relational chain, Property_Listing → Address_Master → Municipality_Master formed the backbone of the AgriSuit database, enabling spatially aware queries that connect market listings to physical land units. The Property_Master table consolidated these relationships into a unified view, providing a single record per property containing its listing details, verified address, and parcel metadata. This relational structure ensured that every property could be unambiguously traced from textual address to geospatial boundary, forming a transparent link between public market information and official land records.

### System Validation and Output

Before finalising the integration, integrity tests were conducted to confirm the accuracy of all joins and key relationships. Duplicate keys, orphaned references, and inconsistent coordinate records were systematically checked and corrected. Spatial overlays were used to verify that linked parcels accurately encompassed their associated geocoded addresses, ensuring no mismatches between tabular and geometric components. The validated output was a harmonised, fully georeferenced relational database capable of supporting both tabular and spatial queries. This structure provided a robust foundation for the analytical operations that follow in Section 6.3, where the database relationships are extended to incorporate environmental suitability data and pixel-based aggregation.
...
2. Probabilistic record linkage. Algorithms such as Jaro–Winkler (Jaro) or Levenshtein distance can detect near-matches in noisy vendor feeds, supporting semi-automated integration where deterministic keys are missing [@andre2024]. 
...
Geospatial inputs consist of layered surfaces and geometries that must be spatially consistent. A raster is a gridded surface in which each cell (pixel) stores a value for a single thematic layer (e.g., enterprise suitability); multiple layers coexist and must share a coordinate reference system to be comparable. In contrast to CSVs or single SQL tables, useful analysis emerges only after geometries (parcels, address points) and rasters are aligned and joined with stable identifiers. In our pipeline, relational keys from Section 6.3 (notably PID and canonical full_address) anchor this alignment, while spatial containment ensures that per-parcel summaries correspond exactly to what is rendered.

The overall suitability at pixel scale follows the Most-Limiting-Factor (MLF) principle, "for each crop, soil, climate, and terrain sub-rules yield categorical classes, and the strictest class governs the pixel’s final label". This non-compensatory operator makes the mapping transparent and risk-averse, so parcel-level tallies faithfully reflect limiting conditions.

Operationally, the parcel geometry is validated and reprojected to the crop raster’s CRS, the raster is windowed to a buffered cut-out around the parcel for efficiency, and the value attribute table (VAT) is used to translate numeric codes into human-readable class names. For pixel counting, the raster is masked by the parcel polygon and intersecting pixels are retrieved with their inside-parcel fractions; a 50% majority rule excludes boundary slice and reduces double-counting at edges. Counts are aggregated by suitability class, then stamped with parcel identifiers (PID), canonical address, municipality, and crop name, yielding a per-parcel x per-crop summary that is written to the analytics table for downstream queries and filtering.

For pixel mapping, the same alignment and VAT translation steps are used, but the goal is an interpretable visualisation. The raster is clipped exactly to the parcel footprint, class labels are applied, and a stable colour scheme from the legend registry is assigned (with a neutral fallback when a class lacks a registered colour). Only classes present within the parcel are retained, producing a clean, colour-coded map tile that corresponds one-to-one with the counts stored in the summary table and the relational bridges.
...
### Integration of AgriSuit with Live Property Listing Pages
...
Beyond its immediate application, AgriSuit illustrates how open government datasets can be repurposed into decision-support infrastructure. The platform provides a working model for embedding environmental intelligence into property technology (PropTech) systems, with implications for agricultural planning, sustainable land-use policy, and evidence-based investment frameworks. By making suitability data transparent and accessible, the system supports Tasmania’s broader goals of informed land allocation and sustainable agricultural development.
...
\nocite{R-pkg-DBI,R-pkg-DT,R-pkg-dplyr,R-pkg-duckdb,R-pkg-exactextractr,
R-pkg-foreign,R-pkg-janitor,R-pkg-kableExtra,R-pkg-knitr,R-pkg-leaflet,
R-pkg-lubridate,R-pkg-purrr,R-pkg-raster,R-pkg-RSQLite,R-pkg-rvest,
R-pkg-sf,R-pkg-shiny,R-pkg-stringr,R-pkg-terra,R-pkg-tidygeocoder,
R-pkg-tidyverse,R-pkg-tools}

\forceprintbibliography[heading=bibintoc,title={References}]

\section{GitHub Repository}
The complete codebase for the AgriSuit project is
hosted on GitHub and can be accessed at the following [link](https://github.com/Azham3033/Matrix_Geoscience_ETC5543).


\section{Appendix}

Code snippet for data wrangling (cleaning raw price to numeric column).
...
parse_price_numeric <- function(price_text) {
  ...
  if (stringr::str_detect(txt,
                          "EOI|EXPRESSION|CONTACT|TENDER|AUCTION|BY 
                  NEGOTIATION|PRICE ON APPLICATION|POA|BY OFFER"))
    return(NA_real_)
  # Flexible match: find first number anywhere, with optional $ 
  # and optional K/M suffix
...
Establishing the database connections (Duck DB).
...
```
 ...
 Consolidating the land informations into one table.

```{sql,eval=FALSE, echo=TRUE, message=FALSE,warning=FALSE}
#| code-overflow: wrap
/* creating the final master table by joining the property listing data with
 cadstral address master table.*/
...
Code snippet for Pixel counting.
...
Code for the pixel level mapping on Rshiny dashboard.

```{r  eval=FALSE, echo=TRUE, message=FALSE,warning=FALSE}
#| code-overflow: wrap
tif_path <- file.path(esm_dir, paste0(crop_nm, ".tif"))
validate(need(file.exists(tif_path), 
              sprintf("Missing GeoTIFF: %s", tif_path)))
...
if (file.exists(vat_path)) {
  ...
  if (length(vcol) == 1L) {
    lcol <- grep("(suitab|class|label|cat|legend|desc|category)", names(vat), 
                 ignore.case = TRUE, value = TRUE)
    if (!length(lcol)) lcol <- vcol
    cat_tbl <- data.frame(ID = vat[[vcol]], label = 
                            trimws(as.character(vat[[lcol]])))
...
canon2 <- function(s) { s <- gsub("\\s+"," ", trimws(s)); 
s <- gsub("soil mgmt","soil management", s, ignore.case=TRUE); tolower(s) }
...
ids_in_levels 
<- if (!is.null(lev_df) && "ID" %in% names(lev_df)) as.character(lev_df$ID) 
else character(0)
...
Rshiny server code where the Rshiny user input fetches data from the duckdb 
database.

```{r  eval=FALSE, echo=TRUE, message=FALSE,warning=FALSE}
#| code-overflow: wrap
# ---- Validation (Explore) ----
has_loc_filter<- reactive({ is_present(input$municipality) 
  || is_present(input$addr) })
has_target_filter <- reactive({ selected_not_all(input$crop) 
  || selected_not_all(input$class) })
...
build_sql <- reactive({
  ...
  if (selected_not_all(input$crop)) {
    where <- c(where, sprintf("UPPER(%s)=UPPER(?crop)",
                              dbQuoteIdentifier(con,"CROP"))); 
    params$crop <- input$crop
  } else if (!is.null(input$crop_category) && input$crop_category != "All") {
    where <- c(where, 
    sprintf("UPPER(%s) IN
    (SELECT UPPER(CROP_NAME) FROM %s WHERE UPPER(CROP_CATEGORY)=UPPER(?cat))",
               dbQuoteIdentifier(con,"CROP"),
  ...
  if (!is.null(input$class) && input$class != "All") {
    where <- 
      c(where, sprintf("UPPER(%s)=UPPER(?class_label)",
                       dbQuoteIdentifier(con,"CLASS_LABEL")));
    params$class_label <- input$class
  }
  if (is_present(input$municipality)) {
    where <- 
      c(where, sprintf("UPPER(%s)=UPPER(?municipality)", 
                       dbQuoteIdentifier(con,"MUNICIPALITY"))); 
    params$municipality <- input$municipality
  }
  if (is_present(input$addr)) {
    where <- 
      c(where, sprintf("UPPER(%s) ILIKE '%%'
                       || UPPER(?addr) || '%%'", 
                       dbQuoteIdentifier(con,"FULL_ADDRESS"))); 
  ...
  select_cols <- 
    paste(dbQuoteIdentifier(con, c("FULL_ADDRESS","MUNICIPALITY",
           "CROP","CLASS_LABEL","PRICE","COUNT","COVERAGE","WEBSITE")),
                       collapse=", ")
  sql <- sprintf("SELECT %s FROM %s %s ORDER BY FULL_ADDRESS LIMIT %d",
                 select_cols, 
                 dbQuoteIdentifier(con, MAIN_TABLE), 
                 where_sql, as.integer(input$limit))
...
data_reactive <- eventReactive(input$run, {
  validate(
    need(has_loc_filter(), 
         "Please select a Municipality or enter 
         a Full Address before running."),
    need(has_target_filter(), 
         "Please select either a specific Crop or a Class label
         (Crop Category alone isn’t enough).")
  )
  b <- build_sql()
