...
ui <- fluidPage(
  ...
  tags$head(
    ...
    tags$style(HTML("
      ...
      /* ===== WMS legend suppression on the Leaflet map ===== */
      /* Hide typical WMS legend images and legend containers, but keep zoom + attribution */
      #map_wms .leaflet-control img[src*='GetLegend'],
      #map_wms .leaflet-control img[src*='GetLegendGraphic'],
      #map_wms .leaflet-control .legend,
      #map_wms .leaflet-control.leaflet-legend {
        display: none !important;
      }
      /* Safety net: hide all other controls except zoom and attribution */
      #map_wms .leaflet-control:not(.leaflet-control-zoom):not(.leaflet-control-attribution) {
        display: none !important;
      }

      /* ===== Option A: make WMS tiles quiet (greyscale + fade) ===== */
      /* Only affect WMS tiles (URLs containing 'WMSServer'); leave basemap alone */
      #map_wms img.leaflet-tile[src*='WMSServer'] {
        filter: grayscale(100%) contrast(110%);
        opacity: 0.35;
        mix-blend-mode: multiply;  /* optional: helps parcel outlines pop */
      }
    ")),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$tbl_rows_selected, ignoreInit = TRUE, {
    ...
    # 2) Set CROP from the table (do NOT touch Crop Category)
    ...
    # 3) NEW – clear class filter so mapping can use all classes
    if (!is.null(input$class) && input$class != "All") {
  ...
  observeEvent(input$reg_submit, {
    ...
    tryCatch({
      ...
      # NEW: brief pause so the user sees the success tick, then full reload
      shinyjs::delay(1200, session$sendCustomMessage("reload_page", "now"))
      
    }, error = function(e) {
      # Clear any leftover backdrop, then show the error modal (this is OK)
      removeModal()
      ...
      # Do NOT removeModal() here — it was closing the popup you just showed
