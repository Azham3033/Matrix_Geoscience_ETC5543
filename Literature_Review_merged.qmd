...
After validation, the address data were standardised to ensure structural uniformity across all datasets. Individual address components—street number, street name, street type, suburb, state, and postcode, were concatenated into a composite FULL_ADDRESS field. This field provided a consistent textual identifier across both the web-scraped listings and the verified address-point dataset described in Section 6.1. The purpose of this step was to harmonise address representation rather than immediately establish relational dependencies, reducing ambiguity when cross-referencing between independent data sources. The outcome was a clean, standardised address framework that ensured one-to-one correspondence between listed properties and their official spatial references.
...
hosted on GitHub and can be accessed at the following
[link](https://github.com/Azham3033/Matrix_Geoscience_ETC5543)  


% Print the bibliography here (and add it to ToC)
\printbibliography[heading=bibintoc,title={References}]

% Prevent any later, template-inserted bibliography from printing again
\makeatletter
\let__origprintbibliography\printbibliography
