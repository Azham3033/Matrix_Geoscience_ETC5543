...
Tasmania was chosen as the starting point for AgriSuit because it offers the right mix of economic importance, data quality, and regional focus needed to demonstrate the model’s potential, with current state accounts and recent sector scorecards indicating agriculture’s substantial contribution to the economy and continued growth momentum (Tasmanian Department of Treasury and Finance [DTF], 2025). Agriculture plays a much larger role in Tasmania’s economy than in most Australian states, and the Agri-Food Scorecard shows the sector’s gross value rising in 2022–23, underscoring relevance for piloting decision tools that can strengthen productivity and regional planning (Department of Natural Resources and Environment Tasmania [NRE Tasmania], 2025).

A key advantage of working in Tasmania is the availability of the Tasmanian Enterprise Suitability Model (TESM), which applies a rule-based “most limiting factor” approach and is delivered at approximately 80 m resolution suitable for farm and regional assessments (Department of Premier and Cabinet [DPAC], 2019). Unlike global frameworks such as FAO’s Global Agro-Ecological Zones (GAEZ) and Agro-MAPS—which prioritize broad comparability at coarser resolutions (~1 km pixel scale)—and regional soil information systems like New Zealand’s S-map, TESM delivers locally calibrated, rule-based suitability at higher resolution with explicit most-limiting-factor logic, enabling parcel-level recommendations aligned to Tasmanian conditions (FAO, 2022; DPAC, 2019). TESM and related Digital Soil Assessment resources explicitly combine soil attributes (e.g., pH, depth, drainage) with climate variables (e.g., frost risk, growing degree days, chill hours) to produce categorical suitability classes, enabling transparent traceability from factor thresholds to parcel-level outcomes (DPAC, 2019; Malone & Kidd, 2015).

Tasmania also stands out for its open access to high-quality spatial data via the LIST (Land Information System Tasmania), which provides integrated land and property information and public interfaces (e.g., LISTdata and LISTmap) used by industry and the general public (NRE Tasmania, n.d.-a). LISTmap exposes authoritative layers such as cadastre, topography, imagery, and natural resource data, supporting transparent, reproducible spatial analysis workflows required by AgriSuit’s integration of suitability and property listing information (NRE Tasmania, n.d.-b). The LIST open data registry confirms the portal’s active open data status, reinforcing continuity and accessibility for implementation (NRE Tasmania, n.d.-c).

Altogether, Tasmania offers a unique combination of economic relevance, technical depth, and data openness, with current state accounts and scorecards affirming sector scale and growth, and TESM/DSA plus LIST infrastructure enabling high-resolution, rules-based modelling integrated with market-facing datasets (DTF, 2025; NRE Tasmania, 2025; DPAC, 2019). Positioning AgriSuit in Tasmania demonstrates the model under a data-rich, fine-resolution regime, while the comparative landscape (FAO GAEZ/Agro-MAPS, S-map) indicates clear pathways to adapt schema and rule-sets for other jurisdictions (FAO, 2022; New Zealand Ministry for Primary Industries, 2023). This alignment makes Tasmania the most suitable region to pilot AgriSuit and demonstrate how localized, fine-resolution suitability mapping can transform land-use and crop planning decisions when connected to real-time property information (DPAC, 2019).


## The Tasmanian Enterprise Suitability Map (TESM)

The [Tasmanian Enterprise Suitability Map (TESM)](https://nre.tas.gov.au/agriculture/investing-in-irrigation/enterprise-suitability-toolkit/enterprise-suitability-maps) represents a mature, data-driven framework for assessing agricultural potential across Tasmania. Originating from the [Wealth from Water](https://www.flinders.tas.gov.au/client-assets/images/Council/Downloads/Agendas/2015.02/Annex%206.%20Item%20A1%20Wealth%20from%20Water%20Brochure.pdf) program initiated in 2010 by the Tasmanian Department of Primary Industries, Parks, Water and Environment, which has since been restructured and renamed as the Department of Natural Resources and Environment Tasmania [(NRE Tas)](https://nre.tas.gov.au/) in collaboration with the Tasmanian Institute of Agriculture (TIA) and the University of Sydney, TESM sought to provide an objective, reproducible, and spatially explicit basis for agricultural development planning. The program was itself a direct response to the Innovation Strategy (West et al., 2009), which identified irrigation expansion as a transformative opportunity for Tasmania’s regional economy. The pilot phase initially covered about 70,000 hectares in the Meander Valley and Midlands, subsequently expanding to state-wide coverage through initiatives such as [Water for Profit](https://www.utas.edu.au/__data/assets/pdf_file/0008/1092068/TIA-Water-for-Profit-Brochure-2.pdf). In this progression, the emphasis shifted from qualitative soil surveys to Digital Soil Mapping (DSM) and Digital Soil Assessment (DSA) techniques that operationalised soil and climate data into decision-support tools for land managers and investors (Kidd, 2020).
...
The suitability evaluation itself employs a deterministic, rule-based framework derived from the Food and Agriculture Organization (FAO, 1976) land-evaluation system and refined through the most-limiting-factor principle articulated by Klingebiel and Montgomery. Each crop or enterprise is governed by a rule set developed jointly by TIA researchers and industry experts, defining optimal and limiting parameter thresholds across the soil–climate–topography continuum, as outlined in @tbl-allvariables. The model classifies each grid cell into four main hierarchical categories, i.e. _well suited_ , _suitable_ , _marginally suitable_ , _unsuitable_ based on the lowest-rated criterion. Thus, if drainage or frost risk is poor even when all other factors are optimal, the overall classification defaults to “unsuitable.” This conservative logic enhances interpretability and ensures that biophysical limitations are not overlooked in investment decisions. The framework also differentiates between _manageable_ and _unmanageable_ constraints, acknowledging that interventions such as liming or artificial drainage can upgrade a site’s effective suitability @Kidd2015-dsa-tas.
...
Phenology-aware or scenario-rich designs (e.g., New Zealand implementations that combine frost risk with bud-break timing) mitigate single-indicator brittleness and make climate sensitivity explicit. Global frameworks (e.g., FAO GAEZ) extend scope to yield and yield-gap analyses with scenario pathways, trading local detail for coverage and comparability. For real-estate linkage, hybrids are attractive because they expose both current classification and scenario response, but they demand clear schema bridges (class labels, epochs, units) and careful downscaling when parcel-level decisions are required.
...
DuckDB has performance speed that suitable for exploratory development and demonstrations, since it provides low-latency analytical queries (e.g., parcel-level pixel aggregations, filterable property views) and seamless input/output with CSV, enabling simple, scriptable data exchange across pipeline stages (Kohn, 2022). 

Constraints of the prototype. DuckDB is not optimised for concurrent multi-user access, role-based security, or high availability. As usage scales, risks emerge around contention, access control, backup/restore guarantees, and operational observability (Mozzillo, 2025). A production instance should migrate to a stand-alone database server or managed cloud service.


## Real-Estate Listing Platforms and Agricultural Land Information 

Mainstream Australian real estate platforms typically emphasize transactional details such as parcel size and tenure but seldom include verified biophysical data like soil chemistry, drainage class, frost metrics, or slope (National Housing Supply Council, 2025). When “suitable crops” are mentioned, these are largely anecdotal, seller-supplied claims with no reference to formal datasets or transparent suitability rules, requiring buyers to manually cross-reference external soil and climate sources (Bhaker, 2025). This fragmented process systematically disadvantages non-GIS specialists who cannot easily assemble soil, climate, and topography evidence to support investment or planning decisions (Lee et al., 2023). Studies highlight a broader lack of standardized, integrated agricultural datasets across property platforms, perpetuating risks of misinformation and reduced market transparency (Dumbrell et al., 2024). The absence of scientifically grounded suitability layers fundamentally undermines credible decision-making in agricultural land investment.
...
The key challenge hindering the integration of advanced land-suitability models with real estate listings lies in institutional and policy barriers that prevent spatial datasets from being shared or aligned across agencies and platforms (Deakin University, 2025; Dale et al., 2025). Critical agricultural, water, and zoning information is managed separately from property marketplaces, resulting in fragmented data flows that leave buyers without access to verified, indicator-driven suitability evidence (Harrison et al., 2023). This disconnect between technical data and transactional platforms preserves a systemic gap in market transparency and inhibits evidence-based agricultural investment decisions.
