...
This section outlines the four datasets integrated into the AgriSuit platform: (i) property listings from Domain.com.au, (ii) crop-specific environmental suitability maps (TESM), (iii) cadastral parcel boundaries from LISTMap, and (iv) address-point data from LISTMap. Analyses were conducted in R (v4.3+) using `rvest`, `dplyr`, `purrr`, `sf`, `terra`, `exactextractr`, and `knitr`, `kableExtra.` The interactive prototype uses shiny and leaflet. Each dataset contributes a unique spatial or semantic dimension that collectively enables parcel-level crop-suitability analysis and property–crop linkage. Table 5.1 summarises the provenance, file characteristics, and licensing status of each dataset.
...
The workflow begins with cadastral parcel data sourced from 29 separate municipality files. Each file contains spatial parcel boundaries and unique parcel identifiers (PIDs). These individual datasets are first combined into a single consolidated table called Municipality_Master. This step ensures that all property parcels across Tasmania can be processed consistently within one unified structure.

Once consolidated, the Municipality_Master table is linked with the statewide address-point dataset through the shared PID field. This dataset contains verified address information for each parcel. By joining on PID, we are able to attach reliable address details to each property polygon, forming a complete view of both the physical boundary and its associated address. The result of this join is stored in the Address_Master table.

Next, this enriched dataset is connected to the Property_Master table using the FULL_ADDRESS field. The Property_Master table contains property listings obtained through web-scraping from Domain.com. These listings include information such as the advertised address, indicative price, suburb, and listing URL. Because both datasets share identical address strings, the FULL_ADDRESS field serves as a practical join key. At this stage, each parcel now contains both cadastral information and real-estate listing attributes.

After integration, the compiled property data is exported to a CSV file and passed into a data-transformation process. In this stage, the parcel geometries are spatially overlaid on crop suitability raster maps from the Tasmanian Enterprise Suitability Model (TESM). Each parcel’s polygon is intersected with the pixel values stored in these raster layers to determine how many pixels fall into each suitability class (well-suited, suitable, marginal, unsuitable) for each crop type. This step effectively quantifies how suitable the land is for agricultural purposes.

The results of this pixel-counting process are then written into a dedicated table named CROP_PROPERTY_COUNT_MASTER. This table stores suitability summaries for every possible combination of parcel and crop, and serves as the analytical foundation for later user queries.

Finally, the RShiny application retrieves data directly from the CROP_PROPERTY_COUNT_MASTER table whenever a user interacts with the interface. Based on the user's filter selections, such as crop type, municipality, or suitability class, the app displays relevant properties, suitability scores, and spatial maps. This creates a seamless experience that connects real-estate listings with scientifically derived crop suitability information in a clear, user-friendly format.
...
![Data Flow Diagram](images/data_flow_diagram.jpg){#fig-flow width=80% fig-align="center"}
...
![AgriSuit Default DIsplay](images/1_default_display.jpg){#fig-flow width=80% fig-align="center"}

When the application is first opened, users are presented with the default dashboard interface under the Explore tab. The layout displays filter controls across the top of the screen, including crop category, specific crop, suitability class, municipality, full address search, and maximum rows. At this stage, both map windows remain inactive until valid inputs are provided, ensuring users are not overwhelmed with information.


### Target Preferences Selection Step (Minimum Criteria Requirement)

![Preferences Selection](images/2_target_preferences.jpg){#fig-flow width=80% fig-align="center"}

Users begin interacting with the system by selecting their target preferences through the dropdown filters. To maintain clarity and reduce unnecessary processing, the dashboard enforces a minimum filter requirement:
...
![Table Output](images/3_tabular_output.jpg){#fig-flow width=80% fig-align="center"}

After satisfying the minimum criteria and pressing the Run button, the dashboard retrieves matching properties from the backend database. The lower-left section displays a tabular view of parcel addresses, suitability classifications, and related attributes. Built-in controls allow users to sort, search, and export results to CSV or Excel formats. Map outputs remain unchanged until a specific parcel is selected.


### Automatic Field Population via Row Selection

![Row Click Feature](images/4_row_selection.jpg){#fig-flow width=80% fig-align="center"}

When a property row is clicked, the system automatically populates the Full Address and Crop fields. This behaviour reduces manual typing, minimises errors, and accelerates navigation. The user can then press Run again to trigger geospatial visualisation based on the selected parcel.

### Full Output Rendering (Dynamic Basemap and Static Parcel Map)

![AgriSuit-Property Listing Hyperlink](images/5.5_property_listing_hyperlink.jpg){#fig-flow width=80% fig-align="center"}


Each returned property includes a “View” control that opens the original listing in a new browser tab. This link-out transitions users from on-platform analysis to off-platform transaction workflows, enabling concrete next steps such as contacting the agent, arranging an inspection, requesting disclosure documents, verifying water access and zoning details, and proceeding with negotiation or offer submission as appropriate.


### Full Output Rendering (Dynamic Basemap and Static Parcel Map)

![AgriSuit Explore Page Output](images/5_full_output_agrisuit.jpg){#fig-flow width=80% fig-align="center"}
...
- The static parcel-level suitability map displays pixel classifications according to TESM suitability categories.
This dual-view approach allows users to assess suitability at both regional and local parcel scales.

### Personalised Alerts Through User Registration

![User Registration Page](images/6_user_registration.jpg){#fig-flow width=80% fig-align="center"}

Users seeking automated notifications can register their preferences through the User Registration tab. Options include preferred crop types, municipalities, suitability classes, price ranges, and alert frequency. When new properties meet these criteria, users receive personalised updates, transforming the process from manual searching to proactive discovery.
...
A practical next step is to make suitability analysis scenario-aware. Integrating the TESM 2030 and 2050 layers as first-class toggles—clearly labelled by epoch, native resolution, and any resampling—would enable like-for-like comparisons between present and projected suitability at parcel and regional scales without altering the baseline logic. Complementing this, uncertainty overlays reported alongside TESM classes—not replacing them—can support risk-aware screening; Monte-Carlo or fuzzy-membership summaries at pixel and parcel levels would expose confidence and boundary sensitivity while preserving the official categorical outputs for compliance and communication.

Broadening scope from “can it grow here?” to “is it viable here?” will require additional inputs that remain auditable. Two priorities stand out: (i) hydrology and water-access indicators (distance to secure water sources, scheme connection, allocation/permit status) and (ii) economic layers (yield proxies or observed yields where available, logistics distance to processors/ports, and indicative price/cost ranges). Together, these additions would support a transparent suitability-plus-viability view aligned with investment decisions.

Operational maturity likewise depends on dependable data operations. Automated refresh and synchronisation of listings and cadastral layers, provenance/version tags that bind map releases to app builds, and service decomposition—API endpoints with cloud-optimised GeoTIFF tiling and caching—will help sustain freshness and performance. A personalised notification module (filters for crop, municipality, class, price) can then shift the tool from pull-only exploration to timely, criteria-driven alerts without altering the analytical logic.

Deployment path (multi-user and secure). A production instance should migrate to a stand-alone database server or managed cloud service—for example, PostgreSQL with PostGIS on AWS RDS, Google Cloud SQL, or Azure Database for PostgreSQL. This stack supports spatial types and indexes (PostGIS), automated backups, point-in-time recovery, encryption at rest/in transit, monitoring/alerting, and elastic compute/storage to meet peak demand.

Operational considerations. A cloud database enables controlled capacity scaling (up/down with load), isolates analytical jobs from the web tier, and formalises data governance (least-privilege access, audit logs, scheduled maintenance windows). The migration plan should also pin data provenance (schema versions, release tags) and formalise refresh jobs for listings and cadastral layers.
...
A pragmatic roadmap follows this sequence: deliver scenario toggles and labelling; add uncertainty overlays with concise summary statistics; introduce water-access and core economic indicators; harden data refresh, provenance, and notifications; execute the database migration; then package a jurisdiction template (rule-pack plus data-adapter kit) for pilots beyond Tasmania.
