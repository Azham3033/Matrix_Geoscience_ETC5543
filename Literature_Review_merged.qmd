...
Initially, this project is still within the prototype stage that demonstrates the feasibility of unifying TESM/LISTmap suitability layers with property data in a single, auditable workflow. Subsequently, the current build implements reproducible polygon–raster aggregation (modal parcel labels with limiting-factor diagnostics), consistent CRS/grid handling with explicit resampling rules, and an open-source stack (R Shiny, Leaflet, DuckDB) that supports inspection, testing, and extension.

Moving forward, the integrated interface could reduce context-switching in due diligence, shorten time-to-insight for non-GIS users, and supply a consistent evidence trail for planning and advisory tasks; however, these prospective benefits require measurement in real workflows before any claims can be made. However, the present outputs reflect deterministic TESM classes without uncertainty propagation, so class flips may occur near rule thresholds (brittleness), parcel-level modal labels may obscure fine-scale hotspots (aggregation bias), and grid/resampling choices can affect edge pixels; climate views are limited to available scenario epochs, external listings introduce refresh/schema/ToS risks, and multi-user performance and portability beyond Tasmanian rule libraries remain untested.

Validation will also proceed via LISTmap cross-checks on sampled parcels, boundary and area-balance QA (including $\geq$ 50% coverage checks), and an explicit error budget that attributes variance to resampling, parcel geometry, and geocoding; small user pilots will collect task-time, SUS, and qualitative pain points prior to any efficiency claims. 

Finally, scalability will be addressed by precomputing parcel summaries, persisting results in Parquet/DuckDB, offloading heavy raster operations to API/background jobs, adopting COG tiling with caching and basic observability (latency and cache-hit metrics), and introducing CI/CD for data refresh; optional uncertainty overlays (Monte-Carlo or fuzzy membership with fuzzy-MLF) will be reported alongside the baseline TESM class with clear labeling of native vs. target resolution and scenario epoch, while provenance/versioning and jurisdiction-templated rule libraries form the subsequent roadmap.


## Future Scope

As AgriSuit moves beyond proof-of-concept, the immediate priority is to make suitability analysis scenario-aware. Integrating the TESM 2030 and 2050 climate projection layers as first-class toggles will let users compare present and projected suitability at both parcel and regional scales without altering baseline logic.

To inform risk-aware decisions, the platform should add uncertainty overlays reported alongside TESM classes. Monte-Carlo or fuzzy-membership summaries at pixel and parcel levels would expose confidence and boundary sensitivity while preserving the official categorical output for compliance and communication.

Addressing TESM’s scope limits requires broadening inputs from _“can it grow here?”_ to _“is it viable here?”_. Few variable additions are pivotal, distance to secure water sources, irrigation scheme connection, and economic layers such as yield proxies or observed yields where available, logistics distance to processors/ports, and indicative price/cost ranges. Together these enable a suitability-plus-viability view that remains transparent and auditable.

Operational maturity depends on dependable data operations. Automated refresh and synchronisation of listings and cadastral layers, provenance/version tags binding map releases to app builds, and service decomposition (API endpoints with cloud-based GeoTIFF tiling and caching) will sustain freshness and performance. A personalised notification module (filters for crop, municipality, class, price) can shift the tool from pull-only exploration to timely, criteria-driven alerts without modifying the underlying analytics.

The same concept is portable to other states, but only with custom setup; jurisdiction-specific rule packs and class vocabularies; data adapters for local soil/climate rasters, CRS and metadata conventions, and licensing; expert validation against local practice; and performance tuning for differing data volumes and service constraints.

A pragmatic roadmap follows from this ordering: deliver scenario toggles and labelling; add uncertainty overlays with concise summary statistics; introduce water-access and core economic indicators; harden data refresh, provenance, and notifications; then package a jurisdiction template (rule-pack plus data-adapter kit) for pilot deployments beyond Tasmania.
